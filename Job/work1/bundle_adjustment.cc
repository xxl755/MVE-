...
float extract_focal_len(const std::string& img_name)
{
    ...
    //exif是图片信息结构体
    sfm::FocalLengthEstimate fl = sfm::extract_focal_length(exif);//根据相机型号查找，相机参数Focal_length/sensor_size
        ...
        , sfm::FeatureSet&feat2)
{
    ...
    {
        if(m12[i]<0)continue;//按索引只取出存在匹配点的特征点

        sfm::Correspondence2D2D c2d2d;
        c2d2d.p1[0] = feat1.positions[i][0];//根据匹配好的特征点索引取出特征点坐标
        c2d2d.p1[1] = feat1.positions[i][1];
        c2d2d.p2[0] = feat2.positions[m12[i]][0];//matches_1_2的value对应图二匹配点的索引
    ...
    sfm::RansacFundamental::Result ransac_fundamental_result;//结构体包括3x3基础矩阵与内点向量
    ransac_fundamental.estimate(corr_all, &ransac_fundamental_result);//找到最多内点组与此时基础矩阵（包括内点索引）
                  ...
                  , float f2)
{
    ...
    sfm::fundamental_least_squares(matches, &F);//利用所有内点，找到基础矩阵
    sfm::enforce_essential_constraints(&F);//利用基础矩阵给出不完全准确本证矩阵,只调整了特征值没有用相机内参矩阵

    // 设置相机1的内参矩阵，只有焦距，没有相对于中心的平移
    pose1->set_k_matrix(f1, 0.0, 0.0);
    // 相机1为参考矩阵，将其姿态设置成单位矩阵

    pose1->init_canonical_form();//R为单位矩阵，t为0向量
    ...
    std::vector<sfm::CameraPose> poses;//poses的内部结构体包括相机内参，旋转，平移
    ...
    for(std::size_t i=0; i< poses.size(); ++i){
        poses[i].K = pose2->K;
        if(sfm::is_consistent_pose(matches[0],*pose1, poses[i]))//传入一组匹配点，相机1世界坐标系姿态，待定相机姿态2
        {
            *pose2 = poses[i];//只用调整相机二旋转平移
            found_pose = true;
            break;
        }
    }
...
main (int argc, char *argv[]){
    ...
    double f1=(innexMatrix.at<float>(0,0)/1200+innexMatrix.at<float>(1,1)/1200)/2;
    double &f2=f1;
    ...
    // 控制图像尺寸,一直尺寸减半
  //  int imgSize=img1->get_pixel_amount();//21026304
    while(img1->get_pixel_amount() > MAX_PIXELS){//MAX_PIXELS 1000000
        img1=core::image::rescale_half_size<uint8_t>(img1);
//        imgSize=img1->get_pixel_amount();//328536
    }
...
//    float f2 = extract_focal_len(argv[2]);
    ...
    //FeatureSet：position二维点位置，colors，两种描述子（描述子向量：二维点位置，scale尺度，orientation主向量，128向量）
    ...
    // 对特征坐标进行归一化，使得求解计算稳定，sift特征点匹配，匹配完毕，且匹配点都是内点；Correspondences2D2D两视角匹配点坐标
    sfm::Correspondences2D2D corrs = sift_feature_matching(feat1, feat2);//返回匹配内点点组位置
...
//    }
    ...
    // 计算相机姿态：初始对就决定了世界坐标系位置，一个相机相机坐标系作为世界坐标系，计算相对姿态
    sfm::CameraPose pose1, pose2;//K,R,T,POSE1世界坐标系
    if(!calc_cam_poses(corrs, &pose1, &pose2, f1, f2))//corrs只用一组点
    {//根据匹配点找到基础矩阵，推出本质矩阵，本质矩阵特征向量获得RT参数，选一组匹配点三角化找到一个三维点，方法二获得正确相机姿态
    ...
    for(int i=0; i<corrs.size(); i++)
    {
        math::Vec3f pt_3d = sfm::triangulate_match(corrs[i], pose1, pose2);//三角量测获得三维点，两点构成A矩阵svd分解，4个值齐次化
...
//    }
//完成初始化过程，相机参数初始值、三维点初始值
    std::ofstream out("./examples/task2/test_ba.txt");
    assert(out.is_open());

    //捆绑调整
    std::vector<sfm::ba::Camera> cams(2);
    out<<"n_cam "<< cams.size()<<std::endl;//向文件输出内容

    cams[0].focal_length = pose1.get_focal_length();//两个方向焦距的均值
   ...
   std::fill(cams[1].distortion, cams[1].distortion + 2, 0.0);
    ...
    {
        sfm::ba::Point3D pt;
        pt.pos[0] = pts_3d[i][0];//将pts_3d中的点坐标向量以Point3D格式存储
        pt.pos[1] = pts_3d[i][1];
        pt.pos[2] = pts_3d[i][2];
        p3ds.push_back(pt);
    }
    ...
    for(int i=0; i< p3ds.size(); i++){//三维点信息输入
        out<<p3ds[i].pos[0]<<" "<<p3ds[i].pos[1]<<" "<<p3ds[i].pos[2]<<std::endl;
    }

    assert(p3ds.size()==corrs.size());//匹配点组数与三维点组数应该相等
    // 观察点
    std::vector<sfm::ba::Observation> observations;
    for (std::size_t i = 0; i < p3ds.size(); ++i)
    {
        sfm::ba::Observation obs1;//观察点结构体，坐标，相机索引，点索引
        obs1.camera_id = 0;
        obs1.point_id = i;
        std::copy(corrs[i].p1, corrs[i].p1+2, obs1.pos);//坐标录入
    ...
    // ba优化
    sfm::ba::BundleAdjustment::Options ba_opts;//LM最多迭代次数50，信赖域半径1
    ba_opts.verbose_output = true;
    ba_opts.lm_mse_threshold = 1e-16;
    ba_opts.lm_delta_threshold = 1e-8;//δ门限
