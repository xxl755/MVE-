...
bool sift_compare(const features::Sift::Descriptor &d1, const features::Sift::Descriptor &d2) {
...
main (int argc, char** argv)
{
    ...
    /* SIFT 特征检测. */
    features::Sift::Descriptors sift_descr;//vector load descreption
    features::Sift::Keypoints sift_keypoints;
    {
        features::Sift::Options sift_options;//类外调用雷内结构体，调用inline函数构造struct
        sift_options.verbose_output = true;
        sift_options.debug_output = true;
        features::Sift sift(sift_options);//sift构造函数，属性options，人为配置属性，orig原图，octave，八阶结构：高斯，差分，梯度，方向，keypoint：包括阶数，层数，坐标，description
        sift.set_image(image);//传入图片指针
