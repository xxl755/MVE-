...
#define TRUST_REGION_RADIUS_INIT (1000)//信赖域初始大小
#define TRUST_REGION_RADIUS_DECREMENT (1.0 / 10.0)//信赖域更新调整倍数
...
void load_data(const std::string& file_name
    ...
    // 加载相机参数
    {
        int n_cams = 0;
        getline(in, line);
        std::stringstream stream(line);//sstream 读入line放在steam，流入word+n—cams
        stream >> word >> n_cams;//stream 的line录入第一行两个元素“n_cam“ ”2”分别放入word与n_cams，先读2流入word，再直接流入n_cams,直到无法再向下
        cams.resize(n_cams);
        for (int i = 0; i < cams.size(); i++) {//遍历两个相机参数初始信息
            getline(in, line);
            std::stringstream stream(line);//读入相机各类参数
            stream >> cams[i].focal_length;//从左向右流入，先获得焦距，按属性与索引位置挨个读入
            stream >> cams[i].distortion[0] >> cams[i].distortion[1];//前一个0放入distortion[1]，后一个0放入distortion[0]
            for (int j = 0; j < 3; j++){
                stream >> cams[i].translation[j];}//齐次坐标平移0 0 0
    ...
    {
        int n_points  = 0;
        getline(in, line);//获得输入文件流in中剩余line
        std::stringstream stream(line);//一共点个数
        stream>>word>>n_points;//同时录入n_points 272
        pts3D.resize(n_points);
        for(int i=0; i<n_points; i++) {
            getline(in, line);
            std::stringstream stream(line);
            stream>>pts3D[i].pos[0]>>pts3D[i].pos[1]>>pts3D[i].pos[2];//连续整行流入，最右侧先流入
        }
    }
    ...
    //加载观察点读入数据流的277行
    {
        ...
        getline(in, line);
        std::stringstream stream(line);
        stream>>word>>n_observations;//n_observations 544
        observations.resize(n_observations);
        for(int i=0; i<observations.size(); i++){
            getline(in, line);//camera_id point_id pos[0] pos[1]
            std::stringstream stream(line);
...
std::size_t block_size, sfm::ba::SparseMatrix<double>* B)//稀疏矩阵计算方法
{
...
/**
 * /descrition 将角轴法转化成旋转矩阵 VV=R*V：R=cos(Θ)I+(1-cos(Θ))(K*K')+sin(Θ)(0,-k3,k2;k3,0,k1;-k2,k1,0)
 * @param r 角轴向量
 * @param m 旋转矩阵
 */
void rodrigues_to_matrix (double const* r, double* m)//角轴向量转为旋转矩阵
{
    /* Obtain angle from vector length. */
    double a = std::sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2]);//角轴向量的模长
    /* Precompute sine and cosine terms. tr(R)=1+2cos(Θ)，这里ct是1-cos(Θ)*/
    double ct = (a == 0.0) ? 0.5f : (1.0f - std::cos(a)) / (2.0 * a);
    double st = (a == 0.0) ? 1.0 : std::sin(a) / a;
    /* R = I + st * K + ct * K^2 (with cross product matrix K). */
    m[0] = 1.0 - (r[1] * r[1] + r[2] * r[2]) * ct;//m0:k1^2+(1-k1^2)cos(Θ)
...
void update_camera (sfm::ba::Camera const& cam,
                                 double const* update, sfm::ba::Camera* out)//更新相机参数
{
    ...
    double rot_update[9];
    rodrigues_to_matrix(update + 6, rot_update);//需要先将角轴向量转为旋转矩阵，将增量得到的矩阵作为右扰动承在原方程右侧
...
update_parameters (DenseVectorType const& delta_x
        , std::vector<sfm::ba::Camera>*cameras
, std::vector<sfm::ba::Point3D>*points)//更新相机，更新三维点
{
    /* Update cameras. */
 ...
 * @param dist
 */
void radial_distort (double* x, double* y, double const* dist)//图像畸变计算
{
...
#pragma omp parallel for
    for (std::size_t i = 0; i < observations->size(); ++i)
    {
        ...
        vector_f->at(i * 2 + 1) = rp[1] * (*flen) - obs.pos[1];//畸变后成像坐标
    }
}

/**
 * \description 得到的冲投影误差向量计算均方误差
 * @param vector_f
 * @return
 */
              ...
              double* point_x_ptr, double* point_y_ptr)
{
    ...
    // 计算中间变量 (x,y)关于(xc, yc, zc)的偏导数,x_deriv_zc=-x/zc=-xc/(zc^2)
...
void analytic_jacobian (SparseMatrixType* jac_cam
    ...
    std::size_t const jacobi_rows = observations.size() * 2;//observations内部结构体包括位置数组，相机索引，点索引
                        //行数x2保证uv都有偏导分配位置
    // 定义稀疏矩阵的基本元素
    SparseMatrixType::Triplets cam_triplets, point_triplets;//两个向量
...
//cam_x_ptr存放重投影u对相机参数偏导，point_x_ptr存放重投影u对三维点参数偏导
        /*观察点对应稀疏比矩阵的行，第i个观察点在雅各比矩阵的位置是2*i, 2*i+1*/
        std::size_t row_x = i * 2 + 0;
        std::size_t row_y = i * 2 + 1;

        /*jac_cam中相机对应的列数为camera_id* n_cam_params*/
        std::size_t cam_col = obs.camera_id * num_cam_params;//同一个三维点的两个相机的二维偏导紧密放置

        /*jac_points中三维点对应的列数为point_id* 3*/
        std::size_t point_col = obs.point_id * 3;
    //    SparseMatrixType::Triplet tt=SparseMatrixType::Triplet(row_x, cam_col +2, cam_x_ptr[2]);
        ...
        }//向量cam_triplets先增加u的偏导后紧连着v在对应变量的偏导之后相同三维点的另一个视角
        // row_x，row_y对应uv与分组(0,1一组，2，3一组，，，)组内相同参数偏导索引紧密连接先u后v，先相机1再相机2，第一组后连第二组
        // cam_col对应uv求偏导的索引，同一组点的索引相机1索引是0~8，后相机是9~17
   ...
   // int camSize=cam_triplets.size();//526*18
 //   int point_tripletsSize=point_triplets.size();//526x6
    if (jac_cam != nullptr) {
       jac_cam->allocate(jacobi_rows, camera_cols);//1052x18
       jac_cam->set_from_triplets(cam_triplets);
    }  //将cam_triplets中的同组相机参数先u后v转为先点内全部u后全部v再下一个组内点，再下一组的结构
 ...
 */
    ...
    // 构造正规方程(J^TJ + lambda*I)delta_x = J^T(x - F)
    SparseMatrixType B, C;// B = Jcc, E = Jcp, C = Jpp,  原J^TJ分四块，E两快，BC各一块
    // B = Jc^T* Jc
    matrix_block_column_multiply(Jc, camera_block_dim, &B);
    // C = Jp^T*Jp
    matrix_block_column_multiply(Jp, 3, &C);
    // E = Jc^T*Jp
    SparseMatrixType E = JcT.multiply(Jp);//Jcx

    /* Assemble two values vectors. *///  J^T(x - F);
    // v = Jc^T(F-x), w = Jx^T(F-x), deta_x = [delta_c; delta_p]
    DenseVectorType v = JcT.multiply(F);
    DenseVectorType w = JpT.multiply(F);
    v.negate_self();//v,w取反，F本身是f(Θ)-x，应为x-f(Θ)
    w.negate_self();
    ...
    /* 添加信赖域 */    //J^TJ+λI,λ：1.0 + 1.0 / trust_region_radius
    C.mult_diagonal(1.0 + 1.0 / trust_region_radius);//Jxx
    B.mult_diagonal(1.0 + 1.0 / trust_region_radius);//Jcc
...
//JΘ=b经过高斯消元,J与b元素改变
    ...
    invert_block_matrix_NxN_inplace(&precond, camera_block_dim);//precond是Jcc的逆

    /* 用共轭梯度法求解相机参数. */
    DenseVectorType delta_y(Jc.num_cols());
    typedef sfm::ba::ConjugateGradient<double> CGSolver;
    CGSolver::Options cg_opts;
    cg_opts.max_iterations = cg_max_iterations;//共轭梯度法限制迭代条件
    cg_opts.tolerance = 1e-20;
    CGSolver solver(cg_opts);
    CGSolver::Status cg_status;
    cg_status = solver.solve(S, rhs, &delta_y, &precond);//共轭梯度求解，delta_y是δc，0~17；对应两个相机所有参数
    ...
    /* Fill output vector. */
    std::size_t const jac_cam_cols = Jc.num_cols();//18个相机参数，9x2
    std::size_t const jac_point_cols = Jp.num_cols();//789个三维点参数，263x3
    ...
    for (std::size_t i = 0; i < jac_point_cols; ++i)
        delta_x->at(jac_cam_cols + i) = delta_z[i];//三维点δΘ在相机参数后面
...
void lm_optimization(std::vector<sfm::ba::Camera>*cameras
    ...
    // 计算初始的均方误差，根据初始值得到的残差
    int sizeF=F.size();//F size=1052;1052包括了一个点的uv两个方向误差,共526个二维点，对应263个三维点
    double current_mse = compute_mse(F);//整个向量都是误差，求均方误差
    initial_mse = current_mse;
    final_mse = current_mse;


    // 设置共轭梯度法的相关参数设置信赖域半径
    trust_region_radius = TRUST_REGION_RADIUS_INIT;//初始信赖域半径1000

    /* Levenberg-Marquard 算法. */
    for (int lm_iter = 0; ; ++lm_iter) {

        // 判断终止条件，重投影误差的均方误差小于一定阈值 1e-16
        ...
        analytic_jacobian(&Jc, &Jp);//稀疏矩阵：对两个相机参数，三维点坐标参数的各点两相机雅克比矩阵拼接构成

        //2.0 预置共轭梯梯度法对正规方程进行求解，得到增量δΘ：delta_x；传入两个稀疏雅克比J，重投影误差fΘ-x
        DenseVectorType delta_x;
        ...
        /* 残差值即均方误差减少表示迭代成功
         * 如果正规方程求解成功，则更新相机参数和三维点坐标，并且增大信赖域的尺寸，
         * 趋近于高斯牛顿法
         */
        if (successful_iteration) {
            ...
            if (delta_mse_ratio < lm_delta_threshold) {
                ...
                break;//终止条件，当迭代均方值误差变化很小，退出迭代
            }

            // 增大信赖域大小，LM更接近牛顿法
            trust_region_radius *= TRUST_REGION_RADIUS_GAIN;
        }//正规方程求解失败包括：正规方程求解失败、残差增大，减小信赖域大小重新计算
        else {
            std::cout << "BA: #" << std::setw(2) << std::left << lm_iter
                  << " failure" << std::right
                  << ", MSE " << std::setw(11) << current_mse
...
//cameras是结构体cams的向量，cams属性：焦距1，畸变array2，平移array3，旋转array3x3
//points是结构体Point3D向量，Point3D属性：pos array3，is_constant bool
//observations是结构体Observation向量，结构体属性：二维点pos[2]，相机索引，观测三维点索引(所有三维点在两个视角内均可见)
