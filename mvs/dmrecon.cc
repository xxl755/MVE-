...
DMRecon::DMRecon(core::Scene::Ptr _scene, Settings const& _settings)
    : scene(_scene)
    , settings(_settings)//构造初始：DMRecon传入scene（views，bundle），settings
{
    core::Scene::ViewList const& mve_views(scene->get_views());
    //常引用，向vector<View::Ptr>的mve_views直接传scene下的ViewList类型views
    ...
    for (std::size_t i = 0; i < mve_views.size(); ++i) {
        ...
        views[i] = mvs::SingleView::create(scene, mve_views[i],
                                           this->settings.imageEmbedding);//创建共享指针views【i】，构建SingleView类指针传入
        //SingleView类指针对象：scene的camera传入视角内外参（扭曲0）data传入各种内外参相关信息，worldtocam传RT，campos传入-R'T
        //SingleView类view传入特定视角参数，source_level传入相片长宽与两个近似内参矩阵K
    }
    // 提取refrence view 并创建图像金字塔，refV不用建新赋值一个共享指针，具有campos、worldtocam，scene，view，source_level,embeding图片名
    SingleView::Ptr refV(views[settings.refViewNr]);//views[settings.refViewNr]与refV是相同的共享指针
    if (refV == nullptr)
        throw std::invalid_argument("Invalid master view");

    /* Prepare reconstruction */
    // 加载图像，并且img_pyramid构建图像金字塔，按比例缩放的图片尺寸以及对应的两个近似内参,并从最小尺寸金字塔开始添加图片直到目标图片尺寸
    refV->loadColorImage(this->settings.scale);
    // 创建相关的depth image, normal image, dz image, conf image深度图像，法向量图像等等
    refV->prepareMasterView(settings.scale);//初始化depthImg，normalImg，dzImg,configImg,传入图片尺寸与初始各图的元素为0
