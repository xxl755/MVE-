...
Matching::remove_inconsistent_matches (Matching::Result* matches)
{
    ...
    {   int& matchi=matches->matches_1_2[i];
        if (matchi < 0)
            continue;
        if (matches->matches_2_1[matches->matches_1_2[i]] != (int)i)
            //matches_1_2不为-1时的value，对应matches_2_1的索引，它的value应是matches_1_2的索引
...
Matching::combine_results(Matching::Result const& sift_result,
    Matching::Result const& surf_result, Matching::Result* result)//都是result结构体，存储两幅图互相匹配点
{
    ...
    result->matches_1_2.insert(result->matches_1_2.end(),
        sift_result.matches_1_2.begin(), sift_result.matches_1_2.end());//result->matches_1_2末位填充，从0开始
    result->matches_1_2.insert(result->matches_1_2.end(),
        surf_result.matches_1_2.begin(), surf_result.matches_1_2.end());//result->matches_1_2末位填充，从sift匹配索引后开始
    ...
    /* Fix offsets. *///surf匹配到的点处的索引要根据匹配点偏置偏移
