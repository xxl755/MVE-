...
public:
    ...
    struct Options
    {
         ...
         */
         // 每阶有效的DoG个数S，默认S=3，则每阶需要的DoG图像个数为S+2, 需要高斯平滑的图像个数
         // 是N=S+3
         ...
         */
         // min_octave可设置为-1，octave的索引，当octave为-1代表图片需要上采样2倍，现在有OCtave-1，Octave0，OCtave1，一直到max_octave索引
        int min_octave;
         ...
         */
         // 进行亚像素精度定位时，对极值点处的DoG值的阈值,确定亚像素位置后还要满足门限才能作为特征点
     ...
     */
     //关键点定义，struct包含Octave数，sample所在有效查分层数，两者确定了该店尺度δ，还有坐标
    struct Keypoint
    {
...
public:
    typedef std::vector<Keypoint> Keypoints;//vector容器的关键点与描述子列表
...
public:
    ...
    void process (void);//in func to get feature
