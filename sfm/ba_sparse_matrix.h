...
SparseMatrix<T>::allocate (std::size_t rows, std::size_t cols)
{
    ...
    this->values.clear();
    this->outer.clear();
    this->inner.clear();// col id of none zero elements
...
SparseMatrix<T>::set_from_triplets (Triplets const& triplets)
{
    ...
    SparseMatrix<T> transposed(this->cols, this->rows);//1052x18
    ...
    for (std::size_t i = 0; i < triplets.size(); ++i) {
        int tripLength=triplets[i].row;
        transposed.outer[triplets[i].row]++;//triplets[i].row统计到出现相同行就+1，outer对应位置填入
    }
    /* Convert amounts to indices with prefix sum. */
    std::size_t sum = 0;
    std::vector<std::size_t> scratch(transposed.outer.size());//用于存储triplets向量，每段开头索引，9个为一组索引开头
    for (std::size_t i = 0; i < transposed.outer.size(); ++i)
    {
        std::size_t const temp = transposed.outer[i];
        transposed.outer[i] = sum;
        scratch[i] = sum;//0,9,18,,,
        sum += temp;
    }
    ...
    for (std::size_t i = 0; i < triplets.size(); ++i)
    {
        ...
        int srow=scratch[t.row]++;//记录一个row出现一次就记住，并加一，分别从0，9，18...开始加
        std::size_t pos = srow;//pos指定稀疏矩阵的位置索引，t.row是相同点的u偏导放一块，然后放v偏导
        transposed.values[pos] = t.value;//先放u，然后在隔9位置后放v对应相同参数偏导，18位后放另一个相机的uv参数，从9开始到17，一组点占36位
        transposed.inner[pos] = t.col;//inner是uv对应的偏导参数索引
    }
//    int ocols=transposed.cols;int orows=transposed.rows;//cols=1052;rows=18
    /* Transpose matrix, implicit sorting of inner indices. */
    *this = transposed.transpose();
//    int tcols=this->cols;//cols 18
//    int trows= this->rows;//rows 1052
//    int empty=0;
}
...
SparseMatrix<T>::transpose (void) const
{
   ...
   // int colsOrig= this->cols;int rowsOrig= this->rows;
    ret.values.resize(this->num_non_zero());
    int valuesSize=ret.values.size();
    ret.inner.resize(this->num_non_zero());
    int innerSize=ret.inner.size();
