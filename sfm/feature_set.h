...
public:
    ...
    enum FeatureTypes
    {
        FEATURE_SIFT = 1 << 0,//左移0位，1
        FEATURE_SURF = 1 << 1,//左移1位，2
        FEATURE_ALL = 0xFF//255
    };
    ...
    struct Options
    {
        ...
        Sift::Options sift_opts;//Sift::Options 结构体属性自动填充
...
public:
    ...
    void compute_features (core::ByteImage::Ptr image);

    // todo normalizes the features positions
...
private:
    void compute_sift (core::ByteImage::ConstPtr image);
    void compute_surf (core::ByteImage::ConstPtr image);
...
FeatureSet::Options::Options (void)
    : feature_types(FEATURE_SIFT)//对Options构造函数中的结构体中的feature_types属性赋值，其余结构体属性自动顺序给入
{
}
