...
{//计算初始重投影误差；设置BA终止条件；利用二维观察点与对应三维点、对应相机信息求雅克比矩阵；
    //代入初值投影误差向量，雅克比矩阵，分块构造增量正规方程，共轭梯度求解增量
    /* Setup linear solver. */
    ...
    this->compute_reprojection_errors(&F);// todo F 是误差向量，连续存放一三维点uv方向投影误差
    double current_mse = this->compute_mse(F);// 计算算残差均方值,所有uv方向残差的平方和后除点个数的均值
    ...
    for (int lm_iter = 0; ; ++lm_iter)
    {
        if (lm_iter + 1 > this->opts.lm_min_iterations
            && (current_mse < this->opts.lm_mse_threshold))//LM退出条件1，当前重投影误差均方差足够小
        {
            LOG_V << "BA: Satisfied MSE threshold." << std::endl;
            break;
        }

        /* Compute Jacobian. Jc是关于相机参数的雅克比矩阵，Jp是关于三维点的雅克比矩阵*/ // todo 计算雅各比矩阵
        SparseMatrixType Jc, Jp;
        switch (this->opts.bundle_mode)//枚举类型同时优化
        {
            /*同时优化相机和三维点*/
            case BA_CAMERAS_AND_POINTS:
                this->analytic_jacobian(&Jc, &Jp);
                //计算稀疏雅克比矩阵相机参数矩阵Jc(2dpoint.size*2,num_camra*9)，三维点矩阵Jp(2dpoint.size*2,2dpoint.size/2 *3)
                break;
            /*固定三维点，只优化相机参数，与雅克比矩阵中三维点相关的元素都为0*/
            case BA_CAMERAS:
        ...
        /* Perform linear step. */
        // 预置共轭梯梯度法进行求解delta_x增量*/
        DenseVectorType delta_x;
        LinearSolver pcg(pcg_opts);//pcg构造增量方程
        LinearSolver::Status cg_status = pcg.solve(Jc, Jp, F, &delta_x);//F是各三维点在uv方向重投影误差
 //       int delta_size=delta_x.size();
        /* Update reprojection errors and MSE after linear step. */
        double new_mse, delta_mse, delta_mse_ratio = 1.0;
        if (cg_status.success)
        {/*计算若调整了的相机和三维点的重投影误差，但相机参数与三维点此处没有被更新；之后确定残差值在缩小才确定信赖域是合适的才更新*/
        ...
        {//误差没有变小，选择增大信赖域LM更接近最速下降，不改变上步误差，不修正相机参数，继续内循环直到损失下降
            LOG_V << "BA: #" << std::setw(2) << std::left << lm_iter
                << " failure" << std::right
                << ", MSE " << std::setw(11) << current_mse
                << ",    " << std::setw(11) << " "
        ...
        /* Check termination due to LM iterations. 是否超出迭代次数*/
        if (lm_iter + 1 < this->opts.lm_min_iterations)
            continue;
...
{//各视角可见的三维点对应二维点的重投影误差计算,是LM算法的δΘ-》delta_x；vector_f连续存放一个点的uv方向重投影误差
    if (vector_f->size() != this->observations->size() * 2)
        vector_f->resize(this->observations->size() * 2);//vector_f一个向量存储uv两个方向冲投影误差

#pragma omp parallel for//接下来的for循环采用多线程循环
    //for (std::size_t i = 0; i < 1; ++i)
    for (std::size_t i = 0; i < this->observations->size(); ++i)//遍历所有视角下可以看见三维点的
    {
        Observation const& obs = this->observations->at(i);//平面点索引与三维点索引点相同
        Point3D const& p3d = this->points->at(obs.point_id);
        Camera const& cam = this->cameras->at(obs.camera_id);

        double const* flen = &cam.focal_length; // 相机焦距，只有焦距是单个数其他都是数组
        ...
        if (delta_x != nullptr)
        {
            ...
            {//delta_x->data() + cam_id表示增量向量偏移找到相机更新的增量；K,T直接相加偏差，R乘以罗德里格斯转角转变的R’
                this->update_camera(cam, delta_x->data() + cam_id, &new_camera);//这里的new_camera不是真更新
        ...
        for (int d = 0; d < 3; ++d)//Xc’=R*Xw
        {
            rp[0] += rot[0 + d] * point[d];
            rp[1] += rot[3 + d] * point[d];
            rp[2] += rot[6 + d] * point[d];
        }
        rp[2] = (rp[2] + trans[2]);//图像归一化坐标系坐标
        rp[0] = (rp[0] + trans[0]) / rp[2];
        rp[1] = (rp[1] + trans[1]) / rp[2];

        /* Distort reprojections. */
        this->radial_distort(rp + 0, rp + 1, dist);//rp + 0利用数组的索引偏置找到元素,畸变后归一化点坐标

        /* Compute reprojection error. 冲投影uv方向偏差*/
        vector_f->at(i * 2 + 0) = rp[0] * (*flen) - obs.pos[0];//rp[0] * (*flen)畸变后物理成像点坐标；u方向误差
...
{
    double const radius2 = *x * *x + *y * *y;//r^2
    double const factor = 1.0 + radius2 * (dist[0] + dist[1] * radius2);//1+k0*r^2+k1*r^2
    *x *= factor;
    *y *= factor;
}
    ...
    SparseMatrixType* jac_points)
{
    ...
    std::size_t const camera_cols = this->cameras->size() * this->num_cam_params;//2*9
    std::size_t const point_cols = this->points->size() * 3;//823*3;xyz
    std::size_t const jacobi_rows = this->observations->size() * 2;//823*2*2;u,v

    // 定义稀疏矩阵的基本元素
    SparseMatrixType::Triplets cam_triplets, point_triplets;
    // 对相机进行优化，每个物理成像匹配点对所有相机参数求偏导
    if (jac_cam != nullptr)
        cam_triplets.reserve(this->observations->size() * 2 * this->num_cam_params);//263*2 *9
    // 对三维点进行优化，每个物理成像匹配点对三维点求偏导
    if (jac_points != nullptr)
        point_triplets.reserve(this->observations->size() * 3 * 2);//263*2 *3,每个三维点元素
...
//稀疏矩阵的列是u对相机各个参数偏导，上下行填充uv对相机参数偏导，行表示观察点位置
...
//稀疏矩阵的对应位置上下行填充uv对三维点偏导，行表示观察点位置
    ...
    double* point_x_ptr, double* point_y_ptr)
{
    ...
    double const px = rx + t[0];//相机坐标系
    double const py = ry + t[1];
    double const pz = rz + t[2];
    double const ix = px / pz;//归一化坐标系
    double const iy = py / pz;//yc
    double const fz = cam.focal_length / pz;//  f/zc
    double const radius2 = ix * ix + iy * iy;//r^2=x^2+y^2
    double const rd_factor = 1.0 + (k[0] + k[1] * radius2) * radius2;

    /* Compute exact camera and point entries if intrinsics are fixed 内参矩阵参数固定*/
    ...
    cam_x_ptr[0] = ix * rd_factor;//u对f偏导
    cam_x_ptr[1] = cam.focal_length * ix * radius2;//u对k0偏导
    cam_x_ptr[2] = cam.focal_length * ix * radius2 * radius2;//u对k1偏导
...
#define JACOBIAN_APPROX_PBA 0
    ...
    // rd--ratial distortion  rad--radius2 ；畸变rd对radius2 r^2=x^2+y^2求偏导
    double const rd_deriv_rad = k[0] + 2.0 * k[1] * radius2;

    double const rad_deriv_px = 2.0 * ix / pz;//radius2对归一化坐标系求偏导ix=px/pz
    ...
    double const ix_deriv_X0 = (r[0] - r[6] * ix) / pz;//Xc=(R|T)Xw,Xc=Px/Pz
    ...
    /*   u= f*distort*x, x=xc/zc, xc=(R00*Xw+R01*Yw+RO2*Zw+t0),distort==1+k0*rr+k1*rr*rr;rr是x，y平方和
     * Compute translation derivatives
     * NOTE: px_deriv_t0 = 1；ix归一化坐标系，px相机坐标系,相机坐标系做中间变量
     */
    //uv对相机坐标系求偏导: u_der_xc=f*(xt1*distort_der_rr*rr_der_xc+distort*xt1_der_xc);相机平移向量的偏导数:cam_x_ptr[3] = u_der_xc*xc_der_t0;
    cam_x_ptr[3] = f * (rd_deriv_px * ix + rd_factor * ix_deriv_px);//u对t0偏导,先对中间变量px相机坐标系求导，再乘px对t0偏导
    ...
    /* 相机旋转矩阵的偏导数，利用罗德里格斯旋转方程，代入微小扰动影响，世界坐标系转相机坐标系时引入扰动，左乘
        u= f*distort*x, x=xc/zc ， (xc,yc,zc)=δR*R*(xw,yw,zw)+T*(xw,yw,zw)  [即为XC=δR*R*RW+T]
        Xc=Xc+Wx*RXw；Wx=[[0,-w2,w1],[w2,0,-w0],[-w1,w0,0]]
        中间变量xc，δR是最终变量w（变量w是角轴向量，3维向量），先用相机坐标系对角轴向量求偏导dxc/dw,对每个wi按中间变量连乘求和
        如w0偏导： u_der_xc*xc_der_w0+u_der_yc*yc_der_w0+u_der_zc*zc_der_w0；链式求导
     * Compute rotation derivatives；式中ri表示角轴向量元素
     * rd_deriv_r0 = rd_deriv_rad * rad_deriv_r0；rd_deriv_rad是畸变对rr求导
     */
...
{//更新向量顺序K,T,R，用罗德里格斯转角乘以前旋转矩阵
    if (opts.fixed_intrinsics)//固定内参，只变外参
    {
