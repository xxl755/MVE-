...
{//di*xi=PiX->AX=0,一个观察点的xy提供两个约束，X三个自由度，两个观察点就够
    /* The algorithm is described in HZ 12.2, page 312. */
    math::Matrix<double, 3, 4> P1, P2;
    pose1.fill_p_matrix(&P1);//将pose结构体元素向P1，P2填充KR,Kt
    ...
    math::Vector<double, 4> x = V.col(3);//取svd分解最后一列作为X点元素(未齐次化)
    return math::Vector<double, 3>(x[0] / x[3], x[1] / x[3], x[2] / x[3]);//X坐标
}
    ...
    std::vector<CameraPose const*> const& poses)
{
    ...
    for (std::size_t i = 0; i < poses.size(); ++i)
    {
        ...
        math::Vec2d p = pos[i];
        math::Matrix<double, 3, 4> p_mat;//3x4投影矩阵
        pose.fill_p_matrix(&p_mat);//p_mat填充POSE的，R|T
    ...
    CameraPose const& pose1, CameraPose const& pose2)
{
    ...
    math::Vector<double, 3> x1 = pose1.R * x + pose1.t;//方法二判定
...
//track任意两个视角进行三角量测
    ...
    math::Vec3f best_pos(0.0f);//遍历所有满足前三条件视角，两两视角三角量测，重建内点，进行重投影误差计算，统计内点个数最多的三维点
    for (std::size_t p1 = 0; p1 < poses.size(); ++p1)
        for (std::size_t p2 = p1 + 1; p2 < poses.size(); ++p2)
        {
            ...
            math::Vec3d tmp_pos = triangulate_track(position_pair, pose_pair);//一对匹配点三角量测的到三维坐标，直接线性变换建立约束矩阵A，svd分解解得齐次坐标对第四位归一化
            ...
            for (std::size_t i = 0; i < poses.size(); ++i)//三维点外参矩阵投影即点在相机坐标系的z应大于0
            {
                ...
                x = poses[i]->K * x;//去除z影响的相机成像坐标系
                math::Vec2d x2d(x[0] / x[2], x[1] / x[2]);
                double error = (positions[i] - x2d).norm();//重投影误差，大于限制就是外点
