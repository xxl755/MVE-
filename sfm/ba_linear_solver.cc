    ...
    DenseVectorType* delta_x)
{
    ...
    else if (has_jac_cams && !has_jac_points)//只优化相机参数
    ...
    DenseVectorType const& values, DenseVectorType* delta_x)
{
    ...
    /* Assemble two values vectors. LM方程右侧*/
    DenseVectorType v = JcT.multiply(F);//bc=JC*F
...
//    SparseMatrixType C_diag = C.diagonal_matrix();
    ...
    /* 添加信赖域λ */
    C.mult_diagonal(1.0 + 1.0 / this->opts.trust_region_radius);//JXX
    B.mult_diagonal(1.0 + 1.0 / this->opts.trust_region_radius);//JCC
    ...
    // S = (Jcc+lambda*Icc) - Jcx*inv(Jxx+ lambda*Ixx)*Jcx
    SparseMatrixType S = B.subtract(E.multiply(C).multiply(ET));
    // rhs = bc-  Jc^T*Jx*inv(Jxx+ lambda*Ixx)*bx
    ...
    cg_status = solver.solve(S, rhs, &delta_y, &precond);//依次传入等号左侧，等号右侧，待求量，阈值调节器precond
    ...
    for (std::size_t i = 0; i < jac_point_cols; ++i)
        delta_x->at(jac_cam_cols + i) = delta_z[i];//相机参数增量后放置三维点增量
...
{//共轭梯度法，针对增量方程求解，增量方程的分块矩阵此时只有JCC，带有JX部分都为0
    ...
    /* Compute RHS. */
    DenseVectorType g = Jt.multiply(F);//bc=JC^T*F
    g.negate_self();

    /* Add regularization to H. */
    H.mult_diagonal(1.0 + 1.0 / this->opts.trust_region_radius);//增加λI的完全JCC

    Status status;
    if (block_size == 0)
    {
        /* Use preconditioned CG using the diagonal of H. */
        SparseMatrixType precond = H.diagonal_matrix();//JCC对角化
        precond.cwise_invert();
        ...
        CGSolver solver(cg_opts);
        CGSolver::Status cg_status;
        cg_status = solver.solve(H, g, delta_x, &precond);//共轭梯度求解增量，等号左侧，等号右侧，
        status.num_cg_iterations = cg_status.num_iterations;
