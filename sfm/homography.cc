...
{//直接线性变换dlt，U1=H*U2
    ...
    /* Create 2Nx9 matrix A. Each correspondence creates two rows in A. 变形后的单应矩阵*/
    ...
    /* Only consider the last column of V as the solution. *///svd分解最后一列是最小特征值对应特征向量是H
    for (int i = 0; i < 9; ++i)
        (*result)[i] = V[i * 9 + 8];
...
//因为P2=H*P1，所以P1=H^-1 *P2
    math::Matrix3d invH = math::matrix_inverse(homography);
    math::Vec3d result = invH * p2;
    result /= result[2];
    double error = (p1 - result).square_norm();//计算p1与p2投影到p1视角的均方误差

    result = homography * p1;
    result /= result[2];
    error += (result - p2).square_norm();//计算p1投影到p2视角的均方误差，并与p2投影误差结合
