...
Intrinsics::compute (core::Scene::Ptr scene, ViewportList* viewports)
{
    ...
    core::Scene::ViewList const& views = scene->get_views();//ViewList是类型名，view是私有属性;
...
void
Intrinsics::init_from_views (core::View::Ptr view, Viewport* viewport)
{
    core::CameraInfo const& camera = view->get_camera();
    if (camera.flen == 0.0f)
    {
