...
//viewport结构体：focal_length，distortion2,pose[KRt],image,features[长，宽,特征点positions，colors，]
    ...
    for (std::size_t i = 0; i < views.size(); ++i)//遍历所有视角
    {
        {
            num_done += 1;
        ...
        core::ByteImage::Ptr image = view->get_byte_image//遍历所有图片
        ...
        // 计算每个视角的特征点
        Viewport* viewport = &viewports->at(i);//建立视角，初始化
//        this->opts.feature_options=
//                FeatureSet::Options::feature_types=FeatureSet::FEATURE_SURF;
        viewport->features.set_options(this->opts.feature_options);//视角特征点用特征点初始化
        viewport->features.compute_features(image);//更新视角features属性：特征点position2，颜色colors【向量3】，两个描述子，特征选项
        // 对图像特征点的位置进行初始化
        viewport->features.normalize_feature_positions();//特征点位置position归一化
