...
{
    /*AF=0，F是A的svd分解的最小特征值对应的特征向量，points_view_1的每一列是一个二维成像坐标
...
//相机2姿态可能在U旋转矩阵基础上绕相机1姿态前后绕z轴90°
    ...
    math::Matrix<double, 3, 3> U, S, V;
    math::matrix_svd(matrix, &U, &S, &V);//本证矩阵E分解利用U,V得到相机姿态

    // This seems to ensure that det(R) = 1 (instead of -1).
    // FIXME: Is this the correct way to do it?
    if (math::matrix_determinant(U) < 0.0)//判断矩阵行列式
        for (int i = 0; i < 3; ++i)
            U(i,2) = -U(i,2);
    ...
    if (!MATH_EPSILON_EQ(math::matrix_determinant(result->at(0).R), 1.0, 1e-3))//旋转矩阵行列式条件
