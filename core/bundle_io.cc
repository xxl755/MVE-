...
load_bundler_ps_intern (std::string const& filename, BundleFormat format)
{
    ...
    std::getline(in, version_string);//in：drews 1.0
    ...
    int num_views = 0;
    int num_features = 0;
    in >> num_views >> num_features;//in： 3 3832；读取视角总数与特征点数
...
//复杂的共享指针创建bundle对象，不采用构造函数
    Bundle::Ptr bundle = Bundle::create();//创建Bundle类型共享指针[共享指针传入bundle指针类]返回给对象bundle

    /* Read all cameras. *///Cameras 是typedef std::vector<CameraInfo> Cameras
    Bundle::Cameras& cameras = bundle->get_cameras();
    cameras.reserve(num_views);
    for (int i = 0; i < num_views; ++i)//读入视角的相机参数
    {
        cameras.push_back(CameraInfo());//CameraInfo()是CameraInfo类默认构造，匿名对象初始化，旋转单位，平移0，焦距0
        CameraInfo& cam = cameras.back();//向刚插入相机信息类填信息
        in >> cam.flen >> cam.dist[0] >> cam.dist[1];
        for (int j = 0; j < 9; ++j)
            in >> cam.rot[j];//一个个的获得元素>>插入矩阵
        for (int j = 0; j < 3; ++j)
            in >> cam.trans[j];
    }
    ...
    /* Read all features. */
    Bundle::Features& features = bundle->get_features();//Feature3D:pos[3],color[3],vector<Feature2D>[view_id,feature_id,pos[2]]
    features.reserve(num_features);
    for (int i = 0; i < num_features; ++i)
    {
        ...
        {
            in >> feature.color[j];
            feature.color[j] /= 255.0f;//color读取后归一化
        }

        /* Read feature references. */
        int ref_amount = 0;
        in >> ref_amount;//三维点匹配二维点总个数
        ...
        for (int j = 0; j < ref_amount; ++j)
        {
             ...
             */
            Bundle::Feature2D ref;//view_id,feature_id,pos[2]
            float dummy_float;
            if (format == BUNDLE_FORMAT_PHOTOSYNTHER)
            {
                in >> ref.view_id >> ref.feature_id;
                in >> dummy_float; // Drop reprojection quality.重投影误差都是0
                std::fill(ref.pos, ref.pos + 2, -1.0f);//二维特征点位置初始化为-1
            }
            else if (format == BUNDLE_FORMAT_NOAHBUNDLER)
            {
                in >> ref.view_id >> ref.feature_id;
