...
public:
    ...
    static Scene::Ptr create (std::string const& path);//智能指针构造
...
{
    Scene::Ptr scene(new Scene);//scene共享指针构造；新建一个地址类显示构造获得该智能指针；std::shared_ptr<T> ptr创建一个空指针
    if(path.empty()) return scene;
    //scene中basedir传入路径path
    scene->load_scene(path);//Scene中view向量填充【meta_data(camera:相机信息)images向量（图片名），path（图片路径）】
    return scene;
}
