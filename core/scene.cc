...
Scene::load_scene (std::string const& base_path)
{
    ...
    this->init_views();//Scene中view向量填充【meta_data(camera:相机信息)images向量（图片名），path（图片路径）】
}
...
Scene::init_views (void)
{
    ...
    std::string views_path = util::fs::join_path(this->basedir,
        CORE_SCENE_VIEWS_DIR);// CORE_SCENE_VIEWS_DIR "views/"
    util::fs::Directory views_dir;
    try
    {
        views_dir.scan(views_path);//views_dir存放路径下所有文件夹
    }
    ...
    for (std::size_t i = 0; i < views_dir.size(); ++i)
    {
        ...
        view->load_view(views_dir[i].get_absolute_name());//根据路径下文件夹，将信息载入view
    ...
    for (std::size_t i = 0; i < temp_list.size(); ++i)
    {
        ...
        if (this->views[id] != nullptr)//将temp_list的view中信息交给scene的view
        {
...
Scene::get_bundle (void)
{
    if (this->bundle == nullptr)
    {
        std::string filename = util::fs::join_path(this->basedir,
            CORE_SCENE_BUNDLE_FILE);//CORE_SCENE_BUNDLE_FILE  "synth_0.out"
        this->bundle = load_mve_bundle(filename);//"synth_0.out"文件读取，顺序：视角数，特征数；焦距，扭曲；R;T;三维点位置，颜色，二维点视角索引，点索引
        this->bundle_dirty = false;
    }
    return this->bundle;
}
